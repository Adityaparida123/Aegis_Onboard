...
const nodeStatusStyles: Record<string, string> = {
  ...
  'In Progress': 'border-sky-300 bg-sky-50',
  'Waiting Approval': 'border-amber-300 bg-amber-50',
...
function TaskNode({ data }: { data: { label: string; department: string; status: string; reason?: string } }) {
  return (
    <div className={`w-60 rounded-xl border-2 p-3 shadow-sm ${nodeStatusStyles[data.status] ?? nodeStatusStyles.Pending}`}>
      <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">{data.department}</div>
      <div className="mt-0.5 text-sm font-semibold text-slate-900">{data.label}</div>
      <div className="mt-1 text-[11px] text-slate-500">{data.status}</div>
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-soft">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-medium text-brand-600">Workflow orchestration</p>
            <h2 className="text-2xl font-semibold text-slate-900">Generate onboarding flows</h2>
          </div>
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            <select
              className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 md:w-64"
      ...
      <div className="grid gap-4 xl:grid-cols-[300px_1fr]">
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-soft">
          <h3 className="px-2 text-sm font-semibold text-slate-900">Workflows</h3>
          {isLoading ? (
            <p className="px-2 py-6 text-sm text-slate-500">Loading workflows…</p>
          ) : workflows.length === 0 ? (
            <p className="px-2 py-6 text-sm text-slate-500">No workflows yet. Create one to see the dependency graph.</p>
          ) : (
              ...
              {workflows.map((workflow) => (
                <li key={workflow._id}>
                  <button
                    ...
                    className={`w-full rounded-xl border p-3 text-left transition ${workflow._id === activeWorkflow?._id ? 'border-brand-300 bg-brand-50' : 'border-slate-200 hover:bg-slate-50'}`}
                  >
                    <p className="text-sm font-semibold text-slate-900">{workflow.title}</p>
                    <p className="mt-1 text-xs text-slate-500">{workflow.summary}</p>
            ...
            {activeWorkflow ? (
              <>
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div>
                    <p className="text-sm font-medium text-brand-600">Dependency graph</p>
                    <h3 className="text-lg font-semibold text-slate-900">{activeWorkflow.title}</h3>
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    <StatusBadge status={activeWorkflow.status ?? 'Pending'} />
                    {activeWorkflow.durationMinutes ? (
                      <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">{activeWorkflow.durationMinutes}m</span>
                    ) : null}
                  </div>
                </div>
                <p className="mt-2 text-sm text-slate-600">{activeWorkflow.summary}</p>

                <div className="mt-4 h-[420px] rounded-xl border border-slate-200 bg-slate-50">
                  {detailLoading ? (
                    <p className="p-6 text-sm text-slate-500">Loading graph…</p>
                  ) : tasks.length === 0 ? (
                    <p className="p-6 text-sm text-slate-500">No tasks generated for this workflow.</p>
                  ) : (
                  ...
                  <div className="mt-4">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {approvals.map((approval: any) => (
                        <span key={approval._id} className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs">
