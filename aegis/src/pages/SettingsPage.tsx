...
function Toggle({ checked, onChange, disabled }: { checked: boolean; onChange: () => void; disabled?: boolean }) {
  return (
    <button
      ...
      disabled={disabled}
      onClick={onChange}
      className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full transition-colors ${checked ? 'bg-brand-600' : 'bg-slate-300'} ${disabled ? 'cursor-not-allowed opacity-50' : ''}`}
    >
...
function SelectField({ label, value, options, onChange }: { label: React.ReactNode; value: string; options: string[]; onChange: (value: string) => void }) {
  return (
    <label className="flex items-center justify-between gap-3">
      ...
      <select
        className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none focus:border-brand-500"
        value={value}
...
export function SettingsPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-soft md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-sm font-medium text-brand-600">Workspace configuration</p>
          <h2 className="text-2xl font-semibold text-slate-900">Settings</h2>
          <p className="mt-1 text-sm text-slate-500">Preferences are stored on this device and apply immediately to your control plane.</p>
        </div>
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 transition hover:bg-slate-50" onClick={handleReset}>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Section icon={<UserCircle2 className="h-5 w-5" />} title="Profile" description="Your identity and display preferences">
          <div className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">{user?.email ?? 'demo@aegis.demo'}</p>
              <p className="mt-0.5 text-xs font-medium text-brand-600">{user?.role ?? 'HR'}</p>
            </div>
          </div>
        ...
        <Section icon={<ShieldCheck className="h-5 w-5" />} title="Security" description="Session and authentication posture">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-slate-500">Require an additional verification code at sign-in.</p>
            </div>
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-slate-500">Automatically sign out after a period of inactivity.</p>
            </div>
            <select
              className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none focus:border-brand-500"
        ...
        <Section icon={<Bell className="h-5 w-5" />} title="Notifications" description="Choose which alerts reach the notification bell">
          <div className="flex items-center justify-between gap-3 border-b border-slate-100 pb-4">
            <div>
              ...
              <p className="text-sm text-slate-500">Master switch for all alerts.</p>
            </div>
          ...
          <div className="flex items-center justify-between gap-3 border-t border-slate-100 pt-4">
            <div>
              <p className="text-sm font-medium text-slate-700">Polling interval</p>
              <p className="text-sm text-slate-500">How often the shell refreshes notifications.</p>
            </div>
            <select
              className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none focus:border-brand-500"
        ...
        <Section icon={<Bot className="h-5 w-5" />} title="Automation" description="Orchestration and provisioning behavior">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-sm font-medium text-slate-700">AI-assisted planning</p>
              <p className="text-sm text-slate-500">Use the coordinator agent to plan and route onboarding workflows.</p>
            </div>
            <Toggle checked={automation.aiAssisted} onChange={() => setAutomation({ aiAssisted: !automation.aiAssisted })} />
          </div>
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-sm font-medium text-slate-700">Auto-provision low-risk access</p>
              <p className="text-sm text-slate-500">Provision standard resources without waiting for approval.</p>
            </div>
        ...
        <Section icon={<Link2 className="h-5 w-5" />} title="Integrations" description="Connected systems used during provisioning">
          {([
            ...
            <div key={item.key} className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <div className="rounded-lg bg-slate-100 p-2 text-slate-600"><item.icon className="h-4 w-4" /></div>
                <div>
                  <p className="text-sm font-medium text-slate-700">{item.label}</p>
                  <p className="text-sm text-slate-500">{item.hint}</p>
                </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-800 to-slate-900 p-6 text-white shadow-soft">
          ...
          <dl className="mt-5 space-y-3 text-sm">
            <div className="flex justify-between"><dt className="text-slate-300">Notifications</dt><dd className="font-medium">{notifications.enabled ? 'On' : 'Off'}</dd></div>
            <div className="flex justify-between"><dt className="text-slate-300">MFA</dt><dd className="font-medium">{security.mfaEnabled ? 'Enabled' : 'Disabled'}</dd></div>
            <div className="flex justify-between"><dt className="text-slate-300">Session timeout</dt><dd className="font-medium">{security.sessionTimeoutMinutes >= 60 ? `${security.sessionTimeoutMinutes / 60}h` : `${security.sessionTimeoutMinutes}m`}</dd></div>
            <div className="flex justify-between"><dt className="text-slate-300">AI planning</dt><dd className="font-medium">{automation.aiAssisted ? 'Active' : 'Manual'}</dd></div>
            <div className="flex justify-between"><dt className="text-slate-300">Low-risk auto-provision</dt><dd className="font-medium">{automation.autoProvisionLowRisk ? 'On' : 'Off'}</dd></div>
            <div className="flex justify-between"><dt className="text-slate-300">Integrations</dt><dd className="font-medium">{Object.values(integrations).filter(Boolean).length} / {Object.keys(integrations).length} connected</dd></div>
          </dl>
